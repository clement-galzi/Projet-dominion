...
public class Player {
    ...
    public ListOfCards getAllCards() {
        ...
        ListOfCards Deck = new ListOfCards(hand);
        Deck.addAll(new ListOfCards(inPlay));
        Deck.addAll(new ListOfCards(draw));
        Deck.addAll(new ListOfCards(discard));
        return Deck;
    }
    ...
    public int getVictoryPoints() {
        int t = 0;
        ListOfCards Deck = this.getAllCards();
        for (Card item : Deck) {
            t = t + item.getVictoryValue(this);
        }
        return t;
    }
    ...
    public void playCard(String cardName) {
        Card actu = this.hand.getCard(cardName);
        if (this.hand.contains(actu)) {
            playCard(actu);
        }
    }
    ...
    /**
     * Le joueur gagne une carte de la réserve dans sa main
     *
     * @param cardName nom de la carte à gagner. S'il existe une carte dans la
     *                 réserve ayant ce nom, cette carte est retirée de la réserve et placée
     ...
     * null} si aucune carte n'a été prise dans la réserve.
     */
    public Card gainFromSupplyToHand(String cardName) {
        Card carte = game.getFromSupply(cardName);
        if (carte != null) {
            game.removeFromSupply(cardName);
            this.hand.add(carte);
    ...
    public Card buyCard(String cardName) {
        Card carte = game.getFromSupply(cardName);
        if ((game.getFromSupply(cardName) != null) && (carte.getCost() <= this.money) && (this.numberOfBuys >= 1)) {

            this.money -= carte.getCost();
            this.numberOfBuys -= 1;
            this.gainFromSupply(carte.getName());
            return carte;
        } else {
            return null;
        }
    ...
    public void addToDraw(Card c) {
        this.draw.add(this.draw.get(this.draw.size() - 1));
        for (int i = 0; i < this.draw.size() - 1; i++) {
            this.draw.set(i + 1, this.draw.get(i));
        }
        this.draw.set(0, c);
    }
    ...
    public void removeFromHand(String c) {
        Card carte = this.getAllCards().getCard(c);
        this.hand.remove(carte);
        if (this.hand.contains(carte)) {
            this.hand.remove(carte);
        }
    }
    /**
    ...
    public void prompt(String instruction, List<String> choices) {
        ...
        for (activePlayerIndex = 0; activePlayerIndex < game.getNumberOfPlayers(); activePlayerIndex++) {
            if (game.getPlayer(activePlayerIndex) == this) {
                break;
            }
        }
    ...
    public void removeFromHandCard(Card c) {
        if (this.hand.contains(c)) {
            this.hand.remove(c);
        }
    ...
    public void removeFromHandString(String c) {
    ...
    public void removeFromDraw(Card c) {
        if (this.draw.contains(c)) {
            this.draw.remove(c);
        }

    }

    public ListOfCards firstCardDraw() {
        ListOfCards list = new ListOfCards();
        list.add(this.draw.get(0));
        list.add(this.draw.get(1));
        return list;
    }
    public int numberInHand() {
        return this.hand.size();
    }
    public Card lastInHand() {
    ...
    public void setBonusSilver(int i) {
this.bonusSilver=i;
    }
