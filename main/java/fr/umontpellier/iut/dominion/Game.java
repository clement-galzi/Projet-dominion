...
public class Game {
    ...
    public List<Player> otherPlayers(Player p) {
        int nbrJoueurs = this.getNumberOfPlayers();
        ArrayList<Player> listeJoueur = new ArrayList<>(nbrJoueurs);
        int indicePlayerCourant = indexOfPlayer(p);
        int indicePlayerAjoute = indicePlayerCourant + 1;
        while (listeJoueur.size() < nbrJoueurs - 1) {
            if (indicePlayerAjoute < nbrJoueurs) {
                listeJoueur.add(this.getPlayer(indicePlayerAjoute));
                indicePlayerAjoute++;
            } else {
                listeJoueur.add(this.getPlayer(indicePlayerAjoute - nbrJoueurs));
                indicePlayerAjoute++;
            }
        }
        return listeJoueur;
     ...
     * @return la carte jeté
     * On suppose que la carte a été enlevé de son emplacement précédent
     */
    public Card trashCard(Card carte) {
    ...
    public ListOfCards availableSupplyCards() {
        ListOfCards listeAvailableSupplyCards = new ListOfCards();
        int i = 0;
        while (i < this.supplyStacks.size()) {

            if (!this.supplyStacks.get(i).isEmpty()) {
                listeAvailableSupplyCards.add(this.supplyStacks.get(i).get(0));
            }
            i++;

        }
        return listeAvailableSupplyCards;
    }
    ...
    public Card getFromSupply(String cardName) {
        int i = 0;
        Card carteRetour = null;
        while (i < this.supplyStacks.size() && carteRetour == null) {
            carteRetour = this.supplyStacks.get(i).getCard(cardName);
            i++;
        }
        return carteRetour;
    }
    ...
    public Card removeFromSupply(String cardName) {
        ...
        if (getFromSupply(cardName) == null) {
            return null;
        } else {
            ...
            int i = 0;
            int j = 0;
            while ((i < this.supplyStacks.size()) && (j == 0)) {
                if (this.supplyStacks.get(i).contains(c)) {
                    this.supplyStacks.get(i).remove(c);
                    j++;
                }
                i++;
            }
